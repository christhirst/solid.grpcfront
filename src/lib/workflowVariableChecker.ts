...
/**
 * Extracts all upstream (upload/outgoing/calculated) variables across a workflow.
 * These are outputs/expressions generated by steps that are available for
 * dashboard conditional rules and display widgets.
 */
export function extractUpstreamVariables(workflow: any): UpstreamVariable[] {
  if (!workflow || !Array.isArray(workflow.steps)) return [];
  const results: UpstreamVariable[] = [];
  const seen = new Set<string>();
      ...
      if (v.direction === "up") {
        ...
        if (!seen.has(key)) {
          ...
          results.push({
            ...
            type: v.type || "query",
            stepId: step.id,
            stepName: step.name || `Step ${sIdx + 1} (${step.type || "step"})`,
          });
        }
      }
    });
