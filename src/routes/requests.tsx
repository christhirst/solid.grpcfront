...
export default function RequestsClient() {
  // Request Type Toggle ("grpc" or "http")
  const [requestType, setRequestType] = createSignal<"grpc" | "http">("grpc");

  // HTTP Request State
  const [httpMethod, setHttpMethod] = createSignal<"GET" | "POST" | "PUT" | "DELETE" | "PATCH">("GET");
  ...
  const parsedChartData = () => {
    if (!response() || (!response().data && !response().response)) return { labels: [], datasets: [] };
  ...
  const sendRequest = async () => {
    if (requestType() === "grpc") {
      ...
      if (!url) {
        setRequestError("URL cannot be empty");
        setIsLoading(false);
        return;
      }

      let parsedBody: any = undefined;
      const requiresBody = method !== "GET" && method !== "DELETE";
      if (requiresBody) {
        try {
          if (requestBody().trim()) {
            parsedBody = JSON.parse(requestBody());
      ...
      try {
        ...
        if (parsedBody && !reqHeaders["Content-Type"]) {
        ...
        if (parsedBody !== undefined) {
          fetchOptions.body = typeof parsedBody === "object" ? JSON.stringify(parsedBody) : String(parsedBody);
        }
        ...
        setHistory((prev) => [
          {
            ...
            method: `${method} ${url}`,
            address: url,
            requestBody: requiresBody ? requestBody() : "",
        ...
        setRequestError(err.message || "HTTP request failed");
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <div class="relative z-10 mx-auto max-w-7xl px-6 py-8">
        ...
        <div class="mb-8 fade-in-up delay-1 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div class="flex items-center gap-3 mb-2">
              ...
              <h1 class="text-2xl font-bold tracking-tight text-white">Requests Client</h1>
            </div>
            <p class="text-sm text-[#8b8b9e]">Choose a protocol, customize your options, and trigger requests immediately.</p>
          </div>

          {/* Core Mode Toggle */}
          <div class="flex p-1 bg-[#12121a] border border-[#1e1e2e] rounded-xl self-start md:self-auto shrink-0 shadow-lg">
            <button
              onClick={() => {
                setRequestType("grpc");
                setRequestBody("{}");
              ...
              class={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all ${
                requestType() === "grpc"
                  ? "bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 text-emerald-400 border border-emerald-500/30 shadow-md"
                  : "text-[#8b8b9e] hover:text-white border border-transparent"
              }`}
            >
              <span>⚡</span> gRPC Protocol
            </button>
            <button
              onClick={() => {
                setRequestType("http");
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 fade-in-up delay-2">
          ...
          <div class="lg:col-span-5 space-y-4">
            ...
            {/* Conditional Cards depending on Active Protocol */}
            <Show when={requestType() === "grpc"}>
              {/* Proto Upload Card */}
              <div class="card p-4">
                <div class="flex items-center justify-between mb-3">
                  ...
                  {/* SAVED PROTOS DROPDOWN SELECTOR */}
                  <Show when={savedProtos() && savedProtos().length > 0}>
                    <select 
                      class="rounded-lg border border-[#1e1e2e] bg-[#12121a] px-2 py-1 text-xs text-[#8b8b9e] focus:border-[#2a2a3e] focus:outline-none max-w-[150px] truncate cursor-pointer hover:text-white transition-colors"
                      onChange={(e) => {
            ...
            <Show when={requestType() === "http"}>
              ...
              <div class="card p-4 space-y-4">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <h2 class="text-sm font-semibold text-white">HTTP Target Endpoint</h2>
                </div>

                <div class="grid grid-cols-12 gap-2">
                  {/* HTTP Method selector */}
                  <div class="col-span-4">
                    <label class="block text-[10px] font-semibold text-[#5a5a6e] mb-1.5 uppercase">Method</label>
                    <select
                      value={httpMethod()}
                      onChange={(e) => setHttpMethod(e.currentTarget.value as any)}
                      class="w-full rounded-lg border border-[#1e1e2e] bg-[#12121a] px-3 py-2 text-sm text-white focus:outline-none focus:border-[#3b82f6] cursor-pointer appearance-none text-center font-bold"
                    >
                  ...
                  <div class="col-span-8">
                    <label class="block text-[10px] font-semibold text-[#5a5a6e] mb-1.5 uppercase">Request URL</label>
                    <input
                      type="text"
                      value={httpUrl()}
                      onInput={(e) => setHttpUrl(e.currentTarget.value)}
                      placeholder="https://api.example.com/data"
                      class="w-full rounded-lg border border-[#1e1e2e] bg-[#12121a] px-3 py-2 text-sm text-white placeholder-[#3a3a4e] focus:outline-none focus:border-[#3b82f6] transition-colors font-mono"
                    />
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                  ...
                  <h2 class="text-sm font-semibold text-white">
                    {requestType() === "grpc" ? "Metadata Headers" : "HTTP Headers"}
                  </h2>
                </div>
                <button
            ...
            <div class="card p-4">
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                  ...
                  <h2 class="text-sm font-semibold text-white">
                    {requestType() === "grpc" ? "Request Message Body" : "HTTP Request Body"}
                  </h2>
                  <Show when={requestType() === "grpc" && currentMethod()}>
                ...
                <button
                  onClick={sendRequest}
                  disabled={isLoading() || (requestType() === "grpc" && (!parsedProto() || !selectedMethod()))}
                  class={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-all ${
                    isLoading() || (requestType() === "grpc" && (!parsedProto() || !selectedMethod()))
              ...
              <textarea
                ...
                disabled={requestType() === "http" && (httpMethod() === "GET" || httpMethod() === "DELETE")}
                class={`w-full min-h-[200px] rounded-xl border border-[#1e1e2e] bg-[#12121a] p-4 text-sm text-[#c8c8d8] font-mono resize-y focus:outline-none focus:border-[#3b82f6] transition-colors ${
                  requestType() === "http" && (httpMethod() === "GET" || httpMethod() === "DELETE") ? "opacity-30 cursor-not-allowed" : ""
                }`}
                spellcheck={false}
                placeholder={requestType() === "http" && (httpMethod() === "GET" || httpMethod() === "DELETE") ? "No body payload is sent with GET / DELETE requests" : '{ "key": "value" }'}
              />
            </div>
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-3 mb-3">
                        ...
                        >
                          ...
                          {response()?.success ? response()?.status || "OK" : `Error ${response()?.grpcCode ?? response()?.statusCode ?? ""}`}
                        </span>
