...
import { createSignal, createEffect, onMount, onCleanup, Show, createResource, createMemo, For, Index } from "solid-js";
...
import { b as useParams, u as useNavigate } from "../../entry-server.js";
import "protobufjs";
import "pathe";
import "radix3";
import "seroval";
...
var _tmpl$ = ["<table", ' class="w-full text-left text-xs text-[#c8c8d8]"><thead class="bg-[#1a1a24] text-[#8b8b9e] sticky top-0 shadow-sm"><tr>', '</tr></thead><tbody class="divide-y divide-[#1e1e2e]">', "</tbody></table>"], _tmpl$2 = ["<div", ' class="overflow-auto max-h-[300px] border border-[#2a2a3a]/50 rounded bg-[#101015] custom-scrollbar">', "</div>"], _tmpl$3 = ["<div", ' class="p-3 text-xs text-[#5a5a6e]">No table data</div>'], _tmpl$4 = ["<th", ' class="px-3 py-2 font-medium border-b border-[#2a2a3e] whitespace-nowrap uppercase text-[10px] tracking-wider">', "</th>"], _tmpl$5 = ["<tr", ' class="hover:bg-[#1a1a24]/50 transition-colors">', "</tr>"], _tmpl$6 = ["<td", ' class="px-3 py-2 border-b border-[#1e1e2e]/50 max-w-[150px] truncate"', ">", "</td>"], _tmpl$7 = ["<div", ' style="position:relative;height:250px;background:#0d0f17;border-radius:8px;overflow:hidden;border:1px solid #2d3356"><div style="width:100%;height:100%;overflow:hidden;cursor:grab;position:relative"><div class="lt-canvas" style="position:absolute;transform-origin:0 0"><div class="lt-axis" style="position:absolute;left:0;height:2px;background:linear-gradient(90deg,transparent,#6c63ff 5%,#6c63ff 95%,transparent);box-shadow:0 0 10px rgba(108,99,255,0.4)"></div></div></div><div style="position:absolute;bottom:5px;right:8px;font-size:8px;color:#4a5273;pointer-events:none">Scroll · Drag</div></div>'], _tmpl$8 = ["<div", ' class="bg-[#101015] p-3 rounded border border-[#2a2a3a]/50" style="', '">', "</div>"], _tmpl$9 = ["<p", ' class="text-xs text-[#5a5a6e]">No valid array data for chart</p>'], _tmpl$0 = ["<p", ' class="text-xs text-[#8b8b9e] animate-pulse">Loading map assets...</p>'], _tmpl$1 = ["<select", ' class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-orange-300 font-mono focus:border-orange-500 focus:outline-none mb-3"', "><option value>Select a source step…</option><!--$-->", "<!--/--></select>"], _tmpl$10 = ["<option", "><!--$-->", "<!--/--> <!--$-->", "<!--/--><!--$-->", "<!--/--></option>"], _tmpl$11 = ["<div", ' class="', '"><div class="flex items-center gap-3"><span class="text-xl">', '</span><div><div class="text-xs font-bold uppercase tracking-wider">', '</div><div class="text-[11px] text-slate-300 font-mono mt-0.5">Required Variables: <!--$-->', '<!--/--></div></div></div><span class="text-[10px] font-semibold uppercase px-2.5 py-1 rounded-full border bg-black/40 border-current opacity-80">', "</span></div>"], _tmpl$12 = ["<div", ' class="card p-4 border border-purple-500/30 bg-purple-950/20 mb-6"><div class="flex items-center justify-between mb-2"><span class="text-xs font-bold uppercase tracking-wider text-purple-300 flex items-center gap-2"><span class="inline-block w-2 h-2 rounded-full bg-purple-400 animate-ping"></span>Live Workflow Stream Events (<!--$-->', '<!--/-->)</span></div><div class="max-h-48 overflow-y-auto font-mono text-xs space-y-1 custom-scrollbar bg-[#0a0a0f] p-3 rounded-lg border border-purple-900/50">', "</div></div>"], _tmpl$13 = ["<div", ' class="absolute right-0 mt-2 w-72 rounded-2xl bg-[#181824] border border-[#2a2a3a] shadow-2xl z-50 overflow-hidden fade-in py-1 divide-y divide-[#2a2a3a]/50"><div class="p-1"><div class="px-3 py-1 text-[10px] font-bold text-blue-400 uppercase tracking-wider flex items-center justify-between"><span>📥 1. SOURCES</span><span class="text-[9px] text-[#5b5b6e] font-normal">(Start Flow)</span></div><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-blue-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>⚡</span> gRPC Request (Source)</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-cyan-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🌐</span> HTTP / REST API (Source)</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-amber-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🗄️</span> Database Query (Source)</button></div><div class="p-1"><div class="px-3 py-1 text-[10px] font-bold text-purple-400 uppercase tracking-wider flex items-center justify-between"><span>🔀 2. TRANSFORMS & SERVICES</span><span class="text-[9px] text-[#5b5b6e] font-normal">(In ➔ Out)</span></div><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-purple-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🔀</span> JSONata Transformer / Merger</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-blue-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>⚡</span> gRPC Service Call (Transform)</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-cyan-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🌐</span> HTTP / REST API Call (Transform)</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-amber-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🗄️</span> Database Mutation / Query</button></div><div class="p-1"><div class="px-3 py-1 text-[10px] font-bold text-emerald-400 uppercase tracking-wider">🎯 3. TARGETS (Terminal Sinks)</div><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-emerald-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>📊</span> View Data Table</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-pink-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>📈</span> Chart Visualizer</button><button type="button" class="w-full text-left px-3 py-1.5 text-xs text-rose-300 hover:bg-[#222234] rounded-lg flex items-center gap-2 transition-colors"><span>🦋</span> AntV Infographic Diagram</button></div></div>'], _tmpl$14 = ["<div", ' class="space-y-4">', "</div>"], _tmpl$15 = ["<div", ' class="flex items-center justify-between pt-4 border-t border-[#2a2a3a]/50"><h3 class="text-sm font-bold uppercase tracking-wider text-[#8b8b9e]">Step Configurations (<!--$-->', '<!--/-->)</h3><span class="text-xs text-[#5b5b6e]">Select or edit step parameters below</span></div>'], _tmpl$16 = ["<div", ' class="rounded-xl border border-dashed border-[#2a2a3a] py-12 text-center bg-[#0a0a0f]/50"><p class="text-[#8b8b9e] text-sm">No steps added yet. Add a step to start your workflow.</p></div>'], _tmpl$17 = ["<div", ' class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-left"><div class="card max-w-lg w-full p-6 space-y-6 shadow-2xl border border-[#2a2a3a] bg-[#0c0c12]"><div class="flex items-center justify-between border-b border-[#2a2a3a] pb-4"><h2 class="text-xl font-bold text-white flex items-center gap-2"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>Workflow Settings</h2><button class="text-[#8b8b9e] hover:text-white transition-colors"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button></div><div class="space-y-4"><div><label class="mb-1 block text-sm font-medium text-[#8b8b9e]">Proto File</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-3 text-sm text-white focus:border-blue-500 focus:outline-none"', "><option value>No Proto File (REST / Table Only)</option><!--$-->", '<!--/--></select><p class="mt-2 text-[10px] text-[#5b5b6e]">Select a saved proto definition file from registry to enable autocomplete and service selectors in steps.</p></div><div><label class="mb-1 block text-sm font-medium text-[#8b8b9e]">Server Address</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-3 text-sm text-white focus:border-blue-500 focus:outline-none" placeholder="e.g. localhost:50051"', '></div><div class="rounded-lg bg-[#0a0a12] border border-emerald-500/15 p-3 space-y-2"><label class="block text-xs font-medium text-emerald-400/80"><span class="inline-flex items-center gap-1.5"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>Trusted CA Certificate</span></label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-emerald-500 focus:outline-none"', '><option value>None</option><option value="__accept_all__">Accept All</option><!--$-->', '<!--/--></select><p class="text-[10px] text-[#5a5a6e]">None uses plaintext. Accept All uses TLS without certificate validation. A selected CA uses TLS and validates the server certificate. <a href="/TrustedCA" class="text-emerald-400/70 hover:text-emerald-400 underline" target="_blank">Manage CAs →</a></p></div><div class="border-t border-[#2a2a3a]/60 pt-4"><label class="mb-1 block text-sm font-medium text-[#8b8b9e]">Schedule (Cron)<span class="ml-1 text-[10px] text-[#5b5b6e]">e.g. */5 * * * *</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-3 text-sm text-white focus:border-blue-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="Leave empty for manual only"', '><p class="mt-2 text-[10px] text-[#5b5b6e]">Uses standard cron syntax (min hour day month weekday).</p></div><div class="border-t border-[#2a2a3a]/60 pt-4"><label class="mb-1 block text-sm font-medium text-[#8b8b9e]">OAuth Connection</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-3 text-sm text-white focus:border-blue-500 focus:outline-none"', "><option value>No Connection (Public / Unauthenticated)</option><!--$-->", '<!--/--></select><p class="mt-2 text-[10px] text-[#5b5b6e]">Selected connection will retrieve an OAuth token and inject it as <code class="text-[#8b8b9e] font-mono">Authorization: Bearer &lt;token&gt;</code>.</p></div></div><div class="pt-4 border-t border-[#2a2a3a] flex justify-end"><button class="btn-primary px-6 py-2.5">Close Settings</button></div></div></div>'], _tmpl$18 = ["<main", ' class="mx-auto max-w-7xl 2xl:max-w-[90rem] px-4 sm:px-6 py-12"><div class="mb-8 flex items-center justify-between"><div><input class="bg-transparent text-3xl font-extrabold tracking-tight text-white border-none outline-none focus:ring-2 focus:ring-blue-500 rounded px-2 -ml-2 transition-all"', ' placeholder="Workflow Name"></div><div class="flex items-center gap-4"><button class="', '"', '>+ Form Variable</button><div class="w-px h-6 bg-[#2a2a3a] mx-1"></div><button class="btn-secondary flex items-center gap-1.5"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>Settings</button><button class="btn-secondary">Save Flow</button><button class="btn-primary flex items-center gap-2"', "><!--$-->", '<!--/-->Run</button><button class="btn-primary bg-purple-600 hover:bg-purple-700 shadow-purple-500/20 flex items-center gap-2"', "><span>📡</span> Stream Run</button></div></div><!--$-->", "<!--/--><!--$-->", '<!--/--><div class="space-y-6"><div class="space-y-6"><!--$-->', '<!--/--><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-[#2a2a3a]"><div class="flex flex-wrap items-center gap-3"><h2 class="text-xl font-bold text-white flex items-center gap-2">Workflow Structure</h2><div class="flex items-center bg-[#12121a] p-1 rounded-xl border border-[#2a2a3a]"><button type="button" class="', '"><span>🕸️ Visual Graph (Rete.js)</span></button><button type="button" class="', '"><span>📋 Step Details List (<!--$-->', '<!--/-->)</span></button></div></div><div class="relative"><button type="button" class="text-sm font-medium text-blue-400 hover:text-blue-300 flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20 transition-all hover:bg-blue-500/20"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>Add Step</button><!--$-->', "<!--/--></div></div><!--$-->", '<!--/--><div class="', '"><!--$-->', "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--></div></div></div><!--$-->", "<!--/--></main>"], _tmpl$19 = ["<svg", ' class="animate-spin h-4 w-4 text-white" viewBox="0 0 24 24" fill="none"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>'], _tmpl$20 = ["<svg", ' width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>'], _tmpl$21 = ["<div", ' class="flex items-start gap-2 border-b border-[#1e1e2e] pb-1 text-slate-300"><span class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-purple-900/60 text-purple-200">', '</span><span class="text-slate-400 text-[10px]">', '</span><span class="truncate">', "</span></div>"], _tmpl$22 = ["<span", ' class="text-[10px] text-[#5b5b6e]">(Finished in <!--$-->', "<!--/-->ms)</span>"], _tmpl$23 = ["<div", ' class="text-xs font-bold text-red-400 bg-red-500/10 border border-red-500/20 px-3 py-1.5 rounded-lg flex items-center gap-1.5">⚠️ Authentication Failed: <!--$-->', "<!--/--></div>"], _tmpl$24 = ["<div", ' class="', '"><div class="flex items-center gap-3"><span class="flex h-2 w-2 relative"><span class="', '"></span><span class="', '"></span></span><span class="text-xs font-bold uppercase tracking-wider">Workflow Run Status: <!--$-->', "<!--/--></span><!--$-->", "<!--/--></div><!--$-->", '<!--/--><button class="text-[10px] font-semibold text-[#8b8b9e] hover:text-white transition-colors self-start md:self-auto">Clear Results</button></div>'], _tmpl$25 = ["<span", ' class="text-[10px] text-[#5b5b6e] italic">No outgoing data flow</span>'], _tmpl$26 = ["<div", ' class="mb-4 p-3 rounded-xl bg-[#141420] border border-[#2a2a3a] space-y-2"><div class="flex items-center justify-between"><span class="text-xs font-bold text-purple-300 flex items-center gap-1.5"><span>🔗 Connected Upstream Sources</span><span class="text-[10px] text-purple-400 font-semibold bg-purple-500/20 px-1.5 py-0.5 rounded border border-purple-500/30"><!--$-->', '<!--/--> connected</span></span><span class="text-[10px] text-[#6b6b7e]">Toggle upstream steps to automatically connect in visual graph</span></div><!--$-->', "<!--/--></div>"], _tmpl$27 = ["<div", ' class="rounded-xl border border-purple-500/30 bg-[#120e1c] p-4 space-y-3"><div class="flex items-center justify-between"><p class="text-[11px] font-bold text-purple-300 uppercase tracking-wider flex items-center gap-1.5"><span>🔀</span> JSONata Transformation Engine</p><span class="text-[10px] text-purple-400/80 font-mono">jsonata v2</span></div><div><label class="mb-1 block text-xs text-[#8b8b9e]">Quick Presets</label><div class="flex flex-wrap gap-1.5"><button type="button" class="px-2 py-1 rounded bg-[#1e1a2a] hover:bg-[#2e2640] border border-purple-500/20 text-[10px] text-purple-200">Pass Through ($)</button><button type="button" class="px-2 py-1 rounded bg-[#1e1a2a] hover:bg-[#2e2640] border border-purple-500/20 text-[10px] text-purple-200">Merge 2 Sources</button><button type="button" class="px-2 py-1 rounded bg-[#1e1a2a] hover:bg-[#2e2640] border border-purple-500/20 text-[10px] text-purple-200">Filter Array</button><button type="button" class="px-2 py-1 rounded bg-[#1e1a2a] hover:bg-[#2e2640] border border-purple-500/20 text-[10px] text-purple-200">Map & Compute</button><button type="button" class="px-2 py-1 rounded bg-[#1e1a2a] hover:bg-[#2e2640] border border-purple-500/20 text-[10px] text-purple-200">Math Aggregate</button></div></div><div><label class="mb-1 flex items-center justify-between text-xs text-[#8b8b9e]"><span>Transformation Expression</span><span class="text-[9px] text-[#5b5b6e]">Variables: $, source, sources, steps.&lt;id&gt;.response</span></label><textarea class="w-full rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] p-3 text-xs text-purple-200 font-mono focus:border-purple-500 focus:outline-none resize-y" rows="6" placeholder="$"', "></textarea></div></div>"], _tmpl$28 = ["<div", ' class="mt-2 rounded-md bg-purple-500/10 border border-purple-500/20 px-3 py-1.5 text-[10px] text-purple-300">', "</div>"], _tmpl$29 = ["<div", ' class="rounded-lg border border-purple-500/20 bg-[#0d0a10] p-4"><p class="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-3 flex items-center gap-1.5"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline></svg>Chart Overrides</p><div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-xs text-[#8b8b9e]"><!--$-->', '<!--/--> <span class="text-[10px] text-[#5b5b6e]">(optional)</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-purple-500 focus:outline-none"', '></div><div><label class="mb-1 block text-xs text-[#8b8b9e]"><!--$-->', '<!--/--> <span class="text-[10px] text-[#5b5b6e]">(optional)</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-purple-500 focus:outline-none" placeholder="Auto-detect"', "></div></div><!--$-->", "<!--/--></div>"], _tmpl$30 = ["<div", ' class="rounded-lg border border-emerald-500/20 bg-[#080f0a] p-4"><p class="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"></rect><path d="M3 9h18M3 15h18M9 3v18"></path></svg>Table Column Overrides</p><p class="text-[10px] text-[#5b5b6e] mb-3">Leave empty to show all keys found in the array. Add keys only when you want to limit or order columns.</p><div class="space-y-2"><!--$-->', '<!--/--><button type="button" class="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1 mt-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>Add column</button></div></div>'], _tmpl$31 = ["<div", ' class="space-y-4 mt-2"><div class="rounded-lg border border-orange-500/20 bg-[#0f0e08] p-4"><p class="text-[10px] font-bold text-orange-400 uppercase tracking-wider mb-3 flex items-center gap-1.5"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><ellipse cx="12" cy="5" rx="9" ry="3"></ellipse><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"></path><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"></path></svg>Data Source</p><!--$-->', '<!--/--><div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-xs text-[#8b8b9e]">Nested Array Path <span class="text-[10px] text-[#5b5b6e]">(lodash dot path)</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-orange-500 focus:outline-none" placeholder="e.g. shares"', '><p class="text-[10px] text-[#5b5b6e] mt-1">Leave empty if the response is already an array. Use dot notation for deeper nesting.</p></div><div class="bg-[#0d0d14] rounded-lg border border-[#2a2a3a] p-2.5"><p class="text-[10px] text-[#5b5b6e] font-mono mb-1">Example response:</p><pre class="text-[10px] text-orange-300 font-mono overflow-x-auto">{ "cash": 98961,\n  "shares": [\n    { "symbol": "ORCL",\n      "count": 271 }\n  ]\n}</pre><p class="text-[10px] text-[#5b5b6e] mt-1">→ Path: <code class="text-orange-400 font-mono">shares</code></p></div></div></div><!--$-->', "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$32 = ["<span", ' class="text-[10px] text-[#5b5b6e] font-mono"><!--$-->', "<!--/-->ms</span>"], _tmpl$33 = ["<div", ' class="flex p-0.5 bg-[#12121a] border border-[#2a2a3a] rounded-lg shadow-sm"><button type="button" class="', '">Response</button><button type="button" class="', '">Payload</button></div>'], _tmpl$34 = ["<span", ' class="text-[10px] text-[#5b5b6e] font-mono"><!--$-->', "<!--/--> rows</span>"], _tmpl$35 = ["<div", ' class="flex items-center justify-between mb-3 border-b border-[#2a2a3a]/30 pb-2"><div class="flex items-center gap-2"><span class="', '">', "</span><!--$-->", "<!--/--></div><!--$-->", "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$36 = ["<p", ' class="text-[10px] text-[#5b5b6e] uppercase mb-1 font-semibold">Rendered Request Payload</p>'], _tmpl$37 = ["<pre", ' class="text-[11px] text-blue-300 font-mono overflow-auto max-h-[220px] bg-[#0a0a0f] p-3 rounded-lg border border-[#2a2a3a]/40 custom-scrollbar whitespace-pre-wrap break-all flex-1">', "</pre>"], _tmpl$38 = ["<p", ' class="text-[10px] text-[#5b5b6e] uppercase mb-1 font-semibold">Response Content</p>'], _tmpl$39 = ["<pre", ' class="', '">', "</pre>"], _tmpl$40 = ["<div", ' class="flex-1 overflow-hidden flex flex-col">', "</div>"], _tmpl$41 = ["<div", ' class="flex-1 overflow-hidden flex flex-col justify-center">', "</div>"], _tmpl$42 = ["<div", ' class="flex-1 flex flex-col justify-start overflow-hidden"><!--$-->', "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$43 = ["<div", ' class="border-t lg:border-t-0 lg:border-l border-[#2a2a3a]/50 pt-4 lg:pt-0 lg:pl-6 flex flex-col h-full min-h-[220px]">', "</div>"], _tmpl$44 = ["<div", ' class="', '"><div class="absolute -left-[14px] -top-[14px] flex h-6 w-6 items-center justify-center rounded-full bg-purple-600 text-xs font-bold text-white ring-4 ring-[#0a0a0f]">', '</div><div class="mb-3 flex items-center justify-between"><span class="', '">', "</span><!--$-->", '<!--/--></div><div class="', '"><div class="space-y-4"><!--$-->', '<!--/--><div class="flex justify-between items-start mb-4"><div class="flex items-center gap-4 flex-1"><div class="w-1/3"><label class="mb-1 block text-xs text-[#8b8b9e]">Step Type</label><select class="w-full bg-[#1e1e2e] text-white font-medium text-sm border-b border-[#2a2a3a] focus:border-blue-500 outline-none pb-1"', '><optgroup label="📥 1. Sources (Start Flow)"><option value="grpc">⚡ gRPC Request</option><option value="rest">🌐 HTTP / REST Request</option><option value="database">🛢️ Database Query</option></optgroup><optgroup label="🔀 2. Transforms (Processing)"><option value="transform">🔀 JSONata Transformer / Merger</option></optgroup><optgroup label="🎯 3. Targets (Terminal Sinks)"><option value="table">📊 View Data Table</option><option value="infographic">🦋 AntV Infographic Diagram</option><option value="bar">📊 Bar Chart</option><option value="line">📈 Line Chart</option><option value="doughnut">🍩 Doughnut Chart</option><option value="pie">🥧 Pie Chart</option><option value="scatter">📉 Scatter Chart</option><option value="timeline">⟳ Timeline</option><option value="choropleth-us">🗺️ US Choropleth Map</option><option value="choropleth-world">🗺️ World Choropleth Map</option></optgroup><option value="bar">📊 Bar Chart</option><option value="line">📈 Line Chart</option><option value="doughnut">🍩 Doughnut Chart</option><option value="pie">🥧 Pie Chart</option><option value="scatter">📉 Scatter Chart</option><option value="timeline">⟳ Timeline</option><option value="choropleth-us">🗺️ US Choropleth Map</option><option value="choropleth-world">🗺️ World Choropleth Map</option></select></div><div class="flex-1"><label class="mb-1 block text-xs text-[#8b8b9e]">Step ID (for variables)</label><input type="text" class="w-full bg-transparent text-white font-mono text-sm border-b border-[#2a2a3a] focus:border-blue-500 outline-none pb-1"', '></div></div><button class="text-[#5b5b6e] hover:text-red-400 ml-4"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></button></div><!--$-->', "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--></div> <!--$-->", "<!--/--></div></div>"], _tmpl$45 = ["<div", ' class="flex flex-wrap gap-2">', "</div>"], _tmpl$46 = ["<div", ' class="mt-2 pt-2 border-t border-[#2a2a3a] flex items-center justify-between"><span class="text-[11px] text-purple-300 font-medium">Multi-Source Merge Strategy:</span><select class="bg-[#1e1e2e] border border-[#2a2a3a] rounded px-2 py-1 text-xs text-white focus:border-purple-500 outline-none"', '><option value="concat">Array Concatenate (concat)</option><option value="merge_object">Object Deep Merge (merge_object)</option><option value="keyed">Keyed by Step ID (keyed)</option></select></div>'], _tmpl$47 = ["<div", ' class="space-y-2"><!--$-->', "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$48 = ["<p", ' class="text-[11px] text-[#5b5b6e] italic">No upstream source/transform steps available. Add a source step first.</p>'], _tmpl$49 = ["<button", ' type="button" class="', '"', '><span class="font-bold">', "</span><span>", '</span><span class="font-mono">', '</span><span class="text-[10px] opacity-70">(<!--$-->', "<!--/-->)</span></button>"], _tmpl$50 = ["<select", ' class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-blue-500 focus:outline-none"', "><option value", ">Choose a saved connection...</option><!--$-->", "<!--/--></select>"], _tmpl$51 = ["<div", ' class="space-y-3"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e] uppercase tracking-wider">Select Saved gRPC Connection</label><!--$-->', "<!--/--></div><!--$-->", '<!--/--><details class="group rounded-xl border border-[#2a2a3a]/60 bg-[#12121c] p-3 text-xs"><summary class="cursor-pointer font-medium text-[#8b8b9e] hover:text-white flex items-center justify-between list-none select-none"><span class="flex items-center gap-1.5"><span class="text-[10px] group-open:rotate-90 transition-transform">▶</span><span>Advanced Connection Overrides (Optional)</span></span><span class="text-[10px] text-[#5b5b6e]">Leave blank to use connection defaults</span></summary><div class="mt-3 pt-3 border-t border-[#2a2a3a] space-y-3"><div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override Server Address</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-blue-500 focus:outline-none" placeholder="Inherited from connection"', '></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override TLS Trust</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-emerald-500 focus:outline-none"', '><option value>Inherit from Connection</option><option value="__accept_all__">Accept All</option><!--$-->', '<!--/--></select></div></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Headers (Metadata) Overrides Template</label><textarea class="h-16 w-full resize-y font-mono text-xs rounded-lg border border-[#2a2a3a] bg-[#151520] p-2 text-emerald-300 focus:border-emerald-500 focus:outline-none" placeholder="{ &quot;custom-header&quot;: &quot;value&quot; }"', "></textarea></div></div></details></div>"], _tmpl$52 = ["<div", ' class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">Server Address</label><input type="text" class="', '" placeholder="', '"', '></div><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">TLS Trust</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-emerald-500 focus:outline-none"', '><option value>None</option><option value="__accept_all__">Accept All</option><!--$-->', "<!--/--></select></div></div><!--$-->", '<!--/--><div><div class="flex items-center justify-between mb-1"><label class="text-xs text-[#8b8b9e]">Headers (Metadata) Template</label><span class="text-[10px] text-blue-400 font-mono">{ "key": "value" }</span></div><textarea class="h-16 w-full resize-y font-mono text-sm rounded-lg border border-[#2a2a3a] bg-[#151520] p-2.5 text-emerald-300 focus:border-emerald-500 focus:outline-none" placeholder="{ &quot;key&quot;: &quot;value&quot; }"', "></textarea></div></div>"], _tmpl$53 = ["<optgroup", ' label="Load a Saved Proto">', "</optgroup>"], _tmpl$54 = ["<optgroup", ' label="Available Services in Proto">', "</optgroup>"], _tmpl$55 = ["<div", ' class="mb-4 space-y-4"><div class="flex flex-wrap items-center justify-between gap-2 p-2 rounded-xl bg-[#141420] border border-[#2a2a3a]"><div class="flex items-center gap-2"><span class="text-xs font-semibold text-[#8b8b9e]">Connection Mode:</span><span class="text-[10px] text-[#5b5b6e]">', '</span></div><div class="inline-flex rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] p-0.5"><button type="button" class="', '">📦 Saved Connection</button><button type="button" class="', '">⚙️ Custom Configuration</button></div></div><!--$-->', "<!--/--><!--$-->", '<!--/--><div class="grid grid-cols-2 gap-4"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">Service</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-blue-500 focus:outline-none"', "><option value", ">Select a service...</option><!--$-->", "<!--/--><!--$-->", '<!--/--></select></div><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">Method</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-blue-500 focus:outline-none disabled:opacity-50"', "><option value disabled>Select a method...</option><!--$-->", '<!--/--></select></div></div><div><div class="flex items-center justify-between mb-1"><label class="text-xs font-semibold text-[#8b8b9e]">Request Payload Template</label><span class="text-[10px] text-blue-400 font-mono">{{ steps.&lt;id>.response }}</span></div><textarea class="h-32 w-full resize-y font-mono text-sm rounded-lg border border-[#2a2a3a] bg-[#151520] p-3 text-emerald-300 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"', "></textarea></div></div>"], _tmpl$56 = ["<div", ' class="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-300 flex items-center justify-between"><span>No saved gRPC connections found in <code>/connections</code>.</span><a href="/connections" target="_blank" class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 rounded-lg font-medium transition-colors">+ Create Connection ↗</a></div>'], _tmpl$57 = ["<option", ">⚡ <!--$-->", "<!--/--><!--$-->", "<!--/--></option>"], _tmpl$58 = ["<span", ' class="ml-2 text-emerald-400 font-semibold bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">TLS</span>'], _tmpl$59 = ["<div", ' class="p-3 rounded-xl bg-gradient-to-r from-blue-500/10 to-indigo-500/5 border border-blue-500/30 text-xs flex items-center justify-between"><div class="flex items-center gap-3"><span class="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/20 text-blue-400 text-sm">⚡</span><div><div class="font-semibold text-white">', '</div><div class="text-[11px] font-mono text-[#8b8b9e]"><!--$-->', "<!--/--><!--$-->", '<!--/--></div></div></div><a href="/connections" target="_blank" class="text-[11px] text-blue-400 hover:text-blue-300 font-medium underline">Manage ↗</a></div>'], _tmpl$60 = ["<option", ">", "</option>"], _tmpl$61 = ["<option", ' value="', '">Load: <!--$-->', "<!--/--></option>"], _tmpl$62 = ["<option", "", ">", "</option>"], _tmpl$63 = ["<option", "", "><!--$-->", "<!--/--> (<!--$-->", "<!--/--> → <!--$-->", "<!--/-->)</option>"], _tmpl$64 = ["<div", ' class="space-y-3"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e] uppercase tracking-wider">Select Saved HTTP Connection</label><!--$-->', "<!--/--></div><!--$-->", '<!--/--><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">Endpoint Path / Relative URL <span class="text-[10px] text-[#5b5b6e]">(appended to base connection URL)</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-blue-500 focus:outline-none" placeholder="e.g. /v1/users or /api/data/{{ form.id }}"', '></div><div class="grid grid-cols-2 gap-4"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">HTTP Method</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-blue-500 focus:outline-none"', '><option value="GET">GET</option><option value="POST">POST</option><option value="PUT">PUT</option><option value="DELETE">DELETE</option><option value="PATCH">PATCH</option></select></div></div><details class="group rounded-xl border border-[#2a2a3a]/60 bg-[#12121c] p-3 text-xs"><summary class="cursor-pointer font-medium text-[#8b8b9e] hover:text-white flex items-center justify-between list-none select-none"><span class="flex items-center gap-1.5"><span class="text-[10px] group-open:rotate-90 transition-transform">▶</span><span>Advanced Connection Overrides (Optional)</span></span><span class="text-[10px] text-[#5b5b6e]">Headers and TLS overrides</span></summary><div class="mt-3 pt-3 border-t border-[#2a2a3a] space-y-3"><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override TLS Trust</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-emerald-500 focus:outline-none"', '><option value>Inherit from Connection</option><option value="__accept_all__">Accept All</option><!--$-->', '<!--/--></select></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Additional Headers Template (JSON)</label><textarea class="h-16 w-full resize-y font-mono text-xs rounded-lg border border-[#2a2a3a] bg-[#151520] p-2 text-emerald-300 focus:border-emerald-500 focus:outline-none" placeholder="{ &quot;X-Custom-Header&quot;: &quot;value&quot; }"', "></textarea></div></div></details></div>"], _tmpl$65 = ["<div", ' class="space-y-4"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">Full URL Template</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-blue-500 focus:outline-none" placeholder="e.g. https://api.stripe.com/v1/customers/{{ form.customerId }}"', '></div><div class="grid grid-cols-2 gap-4"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">HTTP Method</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-blue-500 focus:outline-none"', '><option value="GET">GET</option><option value="POST">POST</option><option value="PUT">PUT</option><option value="DELETE">DELETE</option><option value="PATCH">PATCH</option></select></div><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e]">TLS Trust</label><select class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white focus:border-emerald-500 focus:outline-none"', '><option value>None</option><option value="__accept_all__">Accept All</option><!--$-->', "<!--/--></select></div></div><!--$-->", '<!--/--><div><div class="flex items-center justify-between mb-1"><label class="text-xs font-semibold text-[#8b8b9e]">Headers Template (JSON)</label><span class="text-[10px] text-blue-400 font-mono">{ "Authorization": "Bearer {{ token }}" }</span></div><textarea class="h-20 w-full resize-y font-mono text-sm rounded-lg border border-[#2a2a3a] bg-[#151520] p-2.5 text-emerald-300 focus:border-emerald-500 focus:outline-none" placeholder="{ &quot;Content-Type&quot;: &quot;application/json&quot; }"', "></textarea></div></div>"], _tmpl$66 = ["<div", '><div class="flex items-center justify-between mb-1"><label class="text-xs font-semibold text-[#8b8b9e]">Request Body Template (JSON)</label><span class="text-[10px] text-blue-400 font-mono">Supports {{ variables }}</span></div><textarea class="h-32 w-full resize-y font-mono text-sm rounded-lg border border-[#2a2a3a] bg-[#151520] p-3 text-emerald-300 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"', "></textarea></div>"], _tmpl$67 = ["<div", ' class="mb-4 space-y-4"><div class="flex flex-wrap items-center justify-between gap-2 p-2 rounded-xl bg-[#141420] border border-[#2a2a3a]"><div class="flex items-center gap-2"><span class="text-xs font-semibold text-[#8b8b9e]">Connection Mode:</span><span class="text-[10px] text-[#5b5b6e]">', '</span></div><div class="inline-flex rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] p-0.5"><button type="button" class="', '">📦 Saved Connection</button><button type="button" class="', '">⚙️ Custom Configuration</button></div></div><!--$-->', "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$68 = ["<div", ' class="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-300 flex items-center justify-between"><span>No saved HTTP connections found in <code>/connections</code>.</span><a href="/connections" target="_blank" class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 rounded-lg font-medium transition-colors">+ Create Connection ↗</a></div>'], _tmpl$69 = ["<option", ">🌐 <!--$-->", "<!--/--><!--$-->", "<!--/--></option>"], _tmpl$70 = ["<span", ' class="ml-2 text-amber-400 font-semibold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">', "</span>"], _tmpl$71 = ["<div", ' class="p-3 rounded-xl bg-gradient-to-r from-blue-500/10 to-cyan-500/5 border border-blue-500/30 text-xs flex items-center justify-between"><div class="flex items-center gap-3"><span class="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/20 text-blue-400 text-sm">🌐</span><div><div class="font-semibold text-white">', '</div><div class="text-[11px] font-mono text-[#8b8b9e]"><!--$-->', "<!--/--><!--$-->", '<!--/--></div></div></div><a href="/connections" target="_blank" class="text-[11px] text-blue-400 hover:text-blue-300 font-medium underline">Manage ↗</a></div>'], _tmpl$72 = ["<select", ' class="w-full rounded-lg border border-[#2a2a3a] bg-[#1e1e2e] p-2.5 text-sm text-white focus:border-red-500 focus:outline-none"', "><option value", ">Choose a saved connection...</option><!--$-->", "<!--/--></select>"], _tmpl$73 = ["<div", ' class="space-y-3"><div><label class="mb-1 block text-xs font-semibold text-[#8b8b9e] uppercase tracking-wider">Select Saved SurrealDB Connection</label><!--$-->', "<!--/--></div><!--$-->", '<!--/--><details class="group rounded-xl border border-[#2a2a3a]/60 bg-[#12121c] p-3 text-xs"><summary class="cursor-pointer font-medium text-[#8b8b9e] hover:text-white flex items-center justify-between list-none select-none"><span class="flex items-center gap-1.5"><span class="text-[10px] group-open:rotate-90 transition-transform">▶</span><span>Advanced Connection Overrides (Optional)</span></span><span class="text-[10px] text-[#5b5b6e]">Leave blank to use connection defaults</span></summary><div class="mt-3 pt-3 border-t border-[#2a2a3a] space-y-3"><div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override URL</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-red-500 focus:outline-none" placeholder="Inherited from connection"', '></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override Namespace</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-red-500 focus:outline-none" placeholder="Inherited from connection"', '></div></div><div class="grid grid-cols-3 gap-3"><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override Database</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-red-500 focus:outline-none" placeholder="Inherited"', '></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override Username</label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-red-500 focus:outline-none" placeholder="Inherited"', '></div><div><label class="mb-1 block text-[11px] text-[#8b8b9e]">Override Password</label><input type="password" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white font-mono focus:border-red-500 focus:outline-none" placeholder="Inherited"', "></div></div></div></details></div>"], _tmpl$74 = ["<div", ' class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="mb-1 block text-xs text-[#8b8b9e]">Connection URL <span class="text-[10px] text-[#5b5b6e]">(supports {{ variables }})</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-red-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="e.g. ws://127.0.0.1:8000/rpc"', '></div><div><label class="mb-1 block text-xs text-[#8b8b9e]">Namespace <span class="text-[10px] text-[#5b5b6e]">(supports {{ variables }})</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-red-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="e.g. solidflow"', '></div></div><div class="grid grid-cols-3 gap-4"><div><label class="mb-1 block text-xs text-[#8b8b9e]">Database Name <span class="text-[10px] text-[#5b5b6e]">(supports {{ variables }})</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-red-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="e.g. main"', '></div><div><label class="mb-1 block text-xs text-[#8b8b9e]">Username <span class="text-[10px] text-[#5b5b6e]">(supports {{ variables }})</span></label><input type="text" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-red-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="e.g. admin"', '></div><div><label class="mb-1 block text-xs text-[#8b8b9e]">Password <span class="text-[10px] text-[#5b5b6e]">(supports {{ variables }})</span></label><input type="password" class="w-full rounded-lg border border-[#2a2a3a] bg-[#1a1a26] p-2.5 text-sm text-white font-mono focus:border-red-500 focus:outline-none placeholder:text-[#5b5b6e]" placeholder="e.g. admin"', "></div></div></div>"], _tmpl$75 = ["<div", ' class="mb-4 space-y-4"><div class="flex flex-wrap items-center justify-between gap-2 p-2 rounded-xl bg-[#141420] border border-[#2a2a3a]"><div class="flex items-center gap-2"><span class="text-xs font-semibold text-[#8b8b9e]">Connection Mode:</span><span class="text-[10px] text-[#5b5b6e]">', '</span></div><div class="inline-flex rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] p-0.5"><button type="button" class="', '">📦 Saved Connection</button><button type="button" class="', '">⚙️ Custom Configuration</button></div></div><!--$-->', "<!--/--><!--$-->", '<!--/--><div><div class="flex items-center justify-between mb-1"><label class="block text-xs font-semibold text-[#8b8b9e] uppercase tracking-wider">SurrealQL Query</label><span class="text-[10px] text-blue-400 font-mono">Supports {{ variables }}</span></div><div class="relative group"><div class="absolute -inset-0.5 rounded-lg bg-gradient-to-r from-red-600 to-orange-500 opacity-20 blur transition group-hover:opacity-40"></div><textarea class="relative w-full h-32 rounded-lg border border-[#2a2a3a] bg-[#0a0a0f] p-3 text-sm font-mono text-red-300 focus:border-red-500 outline-none custom-scrollbar" placeholder="SELECT * FROM users WHERE age > {{ steps.auth.response.min_age }};"', "></textarea></div></div></div>"], _tmpl$76 = ["<div", ' class="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-300 flex items-center justify-between"><span>No saved SurrealDB connections found in <code>/connections</code>.</span><a href="/connections" target="_blank" class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 rounded-lg font-medium transition-colors">+ Create Connection ↗</a></div>'], _tmpl$77 = ["<option", ">🛢️ <!--$-->", "<!--/--><!--$-->", "<!--/--></option>"], _tmpl$78 = ["<span", ' class="ml-2 text-emerald-400 font-semibold">ns: <!--$-->', "<!--/--> / db: <!--$-->", "<!--/--></span>"], _tmpl$79 = ["<div", ' class="p-3 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/5 border border-emerald-500/30 text-xs flex items-center justify-between"><div class="flex items-center gap-3"><span class="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-500/20 text-emerald-400 text-sm">🛢️</span><div><div class="font-semibold text-white">', '</div><div class="text-[11px] font-mono text-[#8b8b9e]"><!--$-->', "<!--/--><!--$-->", '<!--/--></div></div></div><a href="/connections" target="_blank" class="text-[11px] text-emerald-400 hover:text-emerald-300 font-medium underline">Manage ↗</a></div>'], _tmpl$80 = ["<span", '>Will render: Region = <code class="font-mono">', '</code> · Value = <code class="font-mono">', "</code></span>"], _tmpl$81 = ["<span", '>Will render: X = <code class="font-mono">', '</code> · Y = <code class="font-mono">', "</code></span>"], _tmpl$82 = ["<div", ' class="flex gap-2"><input type="text" class="flex-1 rounded-lg border border-[#2a2a3a] bg-[#1a1a26] px-3 py-1.5 text-sm text-white font-mono focus:border-emerald-500 focus:outline-none" placeholder="key name, e.g. symbol"', '><button type="button" class="text-[#5b5b6e] hover:text-red-400 px-2"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></button></div>'], _tmpl$83 = ["<svg", ' class="animate-spin h-5 w-5 text-blue-500 mb-2" viewBox="0 0 24 24" fill="none"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>'], _tmpl$84 = ["<span", ' class="text-xs text-blue-400 animate-pulse font-medium">Waiting to execute...</span>'], _tmpl$85 = ["<div", ' class="flex flex-col items-center justify-center h-full py-8 text-center text-[#5b5b6e]">', "</div>"], _tmpl$86 = ["<div", ' class="flex flex-col items-center"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="mb-2 text-[#3a3a4e]"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg><span class="text-xs italic">Step not executed</span></div>'], _tmpl$87 = ["<div", ' class="text-[11px] text-[#5b5b6e] italic py-8 text-center border border-dashed border-[#2a2a3a] rounded-lg flex-1 flex items-center justify-center">No table rows found</div>'], _tmpl$88 = ["<div", ' class="text-[11px] text-[#5b5b6e] italic py-8 text-center border border-dashed border-[#2a2a3a] rounded-lg flex-1 flex items-center justify-center">No chart points found</div>'], _tmpl$89 = ["<div", ' class="grid grid-cols-2 gap-3 max-w-md"><div><label class="mb-1 block text-[10px] text-[#8b8b9e]">Username</label><input type="text" class="w-full rounded border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-blue-500 focus:outline-none" placeholder="Username"', '></div><div><label class="mb-1 block text-[10px] text-[#8b8b9e]">Password</label><input type="password" class="w-full rounded border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-blue-500 focus:outline-none" placeholder="Password"', "></div></div>"], _tmpl$90 = ["<p", ' class="text-[10px] text-emerald-400 font-mono mt-1 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Returns token key: <span class="bg-[#1e1e2e] px-1.5 py-0.5 rounded border border-[#2a2a3a] text-white font-bold">', "</span></p>"], _tmpl$91 = ["<div", ' class="max-w-md space-y-2"><label class="mb-1 block text-[10px] text-[#8b8b9e]">Select OAuth Connection</label><select class="w-full rounded border border-[#2a2a3a] bg-[#1a1a26] p-2 text-xs text-white focus:border-blue-500 focus:outline-none"', "><option value disabled>Select Connection...</option><!--$-->", "<!--/--></select><!--$-->", "<!--/--></div>"], _tmpl$92 = ["<div", ' class="mt-4 border-t border-[#2a2a3a]/60 pt-4 text-left"><label class="mb-2 block text-xs font-semibold text-[#8b8b9e] uppercase tracking-wider">Authentication</label><div class="flex p-1 bg-[#151520] rounded-lg max-w-xs mb-3"><button class="', '">None</button><button class="', '">Basic</button><button class="', '">OAuth</button></div><!--$-->', "<!--/--><!--$-->", "<!--/--></div>"];
    ...
    if (data) {
      ...
      const normalizedSteps = rawSteps.map((s, idx) => {
        ...
        return {
          ...
          columns: Array.isArray(s.columns) ? s.columns.filter((c) => typeof c === "string" && c.trim().length > 0) : s.columns
        };
      });
      setSteps(reconcile(normalizedSteps));
      if (!isServer && !isNew && !runData()) {
        const cleanId = (data.id || params.id).replace(/[⟨⟩]/g, "").replace(/^workflow:/, "");
        fetch(`/api/workflows/${cleanId}/run`).then((res) => res.json()).then((json) => {
  ...
  return ssr(_tmpl$18, ssrHydrationKey(), ssrAttribute("value", escape(name(), true), false), `px-3 py-1.5 rounded-lg text-xs font-bold text-white transition-all ${hasActiveInput() ? "bg-purple-600 hover:bg-purple-500 shadow-md ring-2 ring-purple-500/50" : "bg-[#2a2a3a] text-[#5b5b6e] cursor-not-allowed"}`, ssrAttribute("title", hasActiveInput() ? "Insert {{ form.variable }} at cursor" : "Select an input field first", false), ssrAttribute("disabled", isNew || isRunning(), true), isRunning() ? _tmpl$19[0] + ssrHydrationKey() + _tmpl$19[1] : _tmpl$20[0] + ssrHydrationKey() + _tmpl$20[1], ssrAttribute("disabled", isNew || isRunning(), true), escape(createComponent(Show, {
    ...
    children: (step, index) => {
      ...
      return (() => {
        ...
        return ssr(_tmpl$44, ssrHydrationKey(), `card p-5 relative border-l-4 ${escape(borderCls(), true)}`, escape(index()) + 1, `text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${escape(badgeCls(), true)}`, escape(badgeText()), escape(createComponent(Show, {
          get when() {
          ...
          get children() {
            return ssr(_tmpl$26, ssrHydrationKey(), escape((step.sourceStepIds || []).length), (() => {
              ...
              return ssr(_tmpl$47, ssrHydrationKey(), escape(createComponent(Show, {
                get when() {
                  return candidateSteps().length > 0;
                },
                get fallback() {
                  return ssr(_tmpl$48, ssrHydrationKey());
                },
                get children() {
                  return ssr(_tmpl$45, ssrHydrationKey(), escape(createComponent(For, {
                    get each() {
                ...
                get children() {
                  return ssr(_tmpl$46, ssrHydrationKey(), ssrAttribute("value", escape(step.mergeStrategy, true) || "concat", false));
                }
              })));
            })());
          }
          ...
          get children() {
            return (() => {
              const isCustom = () => {
                ...
                return !(connections() || []).some((c) => c.type === "grpc");
              };
              return ssr(_tmpl$55, ssrHydrationKey(), isCustom() ? "Directly configuring host & credentials for this step" : "Using preconfigured profile from /connections", `px-3 py-1 text-xs rounded-md font-medium transition-all ${!isCustom() ? "bg-blue-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, `px-3 py-1 text-xs rounded-md font-medium transition-all ${isCustom() ? "bg-blue-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, escape(createComponent(Show, {
                get when() {
                  return !isCustom();
                },
                get children() {
                  return ssr(_tmpl$51, ssrHydrationKey(), escape(createComponent(Show, {
                    get when() {
                      return (connections() || []).filter((c) => c.type === "grpc").length > 0;
                    },
                    get fallback() {
                      return ssr(_tmpl$56, ssrHydrationKey());
                    },
                    get children() {
                      return ssr(_tmpl$50, ssrHydrationKey(), ssrAttribute("value", escape(step.connectionId, true) || "", false), ssrAttribute("disabled", Boolean(step.connectionId), true), escape(createComponent(For, {
                        get each() {
                          return (connections() || []).filter((c) => c.type === "grpc");
                        },
                        children: (conn) => ssr(_tmpl$57, ssrHydrationKey() + ssrAttribute("value", escape(conn.id, true), false), escape(conn.name), conn.serverAddress ? ` (${escape(conn.serverAddress)})` : "")
                      })));
                    }
                  })), escape(createComponent(Show, {
                    get when() {
                    ...
                    get children() {
                      return (() => {
                        ...
                        return createComponent(Show, {
                          ...
                          get children() {
                            return ssr(_tmpl$59, ssrHydrationKey(), escape(conn().name), escape(conn().serverAddress), escape(createComponent(Show, {
                              get when() {
                                return conn().caId;
                              },
                              get children() {
                                return ssr(_tmpl$58, ssrHydrationKey());
                              }
                            })));
                          }
                        });
                  ...
                  })), ssrAttribute("value", escape(step.serverAddress, true) || "", false), ssrAttribute("value", escape(step.caId, true) ?? "", false), escape(createComponent(For, {
                    get each() {
                      return savedCas();
                    },
                    children: (ca) => ssr(_tmpl$60, ssrHydrationKey() + ssrAttribute("value", escape(ca.id, true), false), escape(ca.name))
              ...
              })), ssrAttribute("value", escape(step.serviceName, true) || "", false), ssrAttribute("disabled", !step.serviceName, true), escape(createComponent(Show, {
                get when() {
                  return savedProtos() && savedProtos().length > 0;
                },
                get children() {
                  return ssr(_tmpl$53, ssrHydrationKey(), escape(createComponent(For, {
                    get each() {
                      return savedProtos();
                    },
                    children: (p) => ssr(_tmpl$61, ssrHydrationKey(), `PROTO:${escape(p.id, true)}`, escape(p.name))
                  })));
                }
              })), escape(createComponent(Show, {
                get when() {
                ...
                get children() {
                  return ssr(_tmpl$54, ssrHydrationKey(), escape(createComponent(For, {
                    get each() {
                      return parsedProto()?.services || [];
                    },
                    children: (svc) => ssr(_tmpl$62, ssrHydrationKey() + ssrAttribute("value", escape(svc.fullName, true), false), ssrAttribute("selected", step.serviceName === svc.fullName, true), escape(svc.fullName))
                  })));
                }
                ...
                children: (m) => ssr(_tmpl$63, ssrHydrationKey() + ssrAttribute("value", escape(m.name, true), false), ssrAttribute("selected", step.methodName === m.name, true), escape(m.name), escape(m.requestType), escape(m.responseType))
              })), ssrAttribute("value", escape(step.requestBodyTemplate, true), false));
            })();
          }
        })), escape(createComponent(Show, {
          get when() {
          ...
          get children() {
            return (() => {
              const isCustom = () => {
                ...
                return !(connections() || []).some((c) => c.type === "http");
              };
              return ssr(_tmpl$67, ssrHydrationKey(), isCustom() ? "Directly configuring host & credentials for this step" : "Using preconfigured profile from /connections", `px-3 py-1 text-xs rounded-md font-medium transition-all ${!isCustom() ? "bg-blue-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, `px-3 py-1 text-xs rounded-md font-medium transition-all ${isCustom() ? "bg-blue-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, escape(createComponent(Show, {
                get when() {
                  return !isCustom();
                },
                get children() {
                  return ssr(_tmpl$64, ssrHydrationKey(), escape(createComponent(Show, {
                    get when() {
                      return (connections() || []).filter((c) => c.type === "http").length > 0;
                    },
                    get fallback() {
                      return ssr(_tmpl$68, ssrHydrationKey());
                    },
                    get children() {
                      return ssr(_tmpl$50, ssrHydrationKey(), ssrAttribute("value", escape(step.connectionId, true) || "", false), ssrAttribute("disabled", Boolean(step.connectionId), true), escape(createComponent(For, {
                        get each() {
                          return (connections() || []).filter((c) => c.type === "http");
                        },
                        children: (conn) => ssr(_tmpl$69, ssrHydrationKey() + ssrAttribute("value", escape(conn.id, true), false), escape(conn.name), conn.url ? ` (${escape(conn.url)})` : "")
                      })));
                    }
                  })), escape(createComponent(Show, {
                    get when() {
                    ...
                    get children() {
                      return (() => {
                        ...
                        return createComponent(Show, {
                          ...
                          get children() {
                            return ssr(_tmpl$71, ssrHydrationKey(), escape(conn().name), escape(conn().url), escape(createComponent(Show, {
                              get when() {
                                return conn().authType && conn().authType !== "none";
                              },
                              get children() {
                                return ssr(_tmpl$70, ssrHydrationKey(), escape(conn().authType));
                              }
                            })));
                          }
                        });
                  ...
                  })), ssrAttribute("value", escape(step.restUrl, true) || "", false), ssrAttribute("value", escape(step.restMethod, true) || "GET", false), ssrAttribute("value", escape(step.caId, true) ?? "", false), escape(createComponent(For, {
                    get each() {
                      return savedCas();
                    },
                    children: (ca) => ssr(_tmpl$60, ssrHydrationKey() + ssrAttribute("value", escape(ca.id, true), false), escape(ca.name))
                ...
                get children() {
                  return ssr(_tmpl$65, ssrHydrationKey(), ssrAttribute("value", escape(step.restUrl, true) || "", false), ssrAttribute("value", escape(step.restMethod, true) || "GET", false), ssrAttribute("value", escape(step.caId, true) ?? escape(caId(), true), false), escape(createComponent(For, {
                    get each() {
                      return savedCas();
                    },
                    children: (ca) => ssr(_tmpl$60, ssrHydrationKey() + ssrAttribute("value", escape(ca.id, true), false), escape(ca.name))
                ...
                get children() {
                  return ssr(_tmpl$66, ssrHydrationKey(), ssrAttribute("value", escape(step.requestBodyTemplate, true) || "", false));
                }
              })));
            })();
          }
          ...
          get children() {
            return ssr(_tmpl$27, ssrHydrationKey(), ssrAttribute("value", escape(step.transformExpression, true) || "$", false));
          }
        })), escape(createComponent(Show, {
          get when() {
          ...
          get children() {
            return (() => {
              const isCustom = () => {
                ...
                return !(connections() || []).some((c) => c.type === "surrealdb");
              };
              return ssr(_tmpl$75, ssrHydrationKey(), isCustom() ? "Directly configuring host & credentials for this step" : "Using preconfigured profile from /connections", `px-3 py-1 text-xs rounded-md font-medium transition-all ${!isCustom() ? "bg-red-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, `px-3 py-1 text-xs rounded-md font-medium transition-all ${isCustom() ? "bg-red-600 text-white shadow-sm" : "text-[#8b8b9e] hover:text-white"}`, escape(createComponent(Show, {
                get when() {
                  return !isCustom();
                },
                get children() {
                  return ssr(_tmpl$73, ssrHydrationKey(), escape(createComponent(Show, {
                    get when() {
                      return (connections() || []).filter((c) => c.type === "surrealdb").length > 0;
                    },
                    get fallback() {
                      return ssr(_tmpl$76, ssrHydrationKey());
                    },
                    get children() {
                      return ssr(_tmpl$72, ssrHydrationKey(), ssrAttribute("value", escape(step.connectionId, true) || "", false), ssrAttribute("disabled", Boolean(step.connectionId), true), escape(createComponent(For, {
                        get each() {
                          return (connections() || []).filter((c) => c.type === "surrealdb");
                        },
                        children: (conn) => ssr(_tmpl$77, ssrHydrationKey() + ssrAttribute("value", escape(conn.id, true), false), escape(conn.name), conn.url ? ` (${escape(conn.url)})` : "")
                      })));
                    }
                  })), escape(createComponent(Show, {
                    get when() {
                    ...
                    get children() {
                      return (() => {
                        ...
                        return createComponent(Show, {
                          ...
                          get children() {
                            return ssr(_tmpl$79, ssrHydrationKey(), escape(conn().name), escape(conn().url), escape(createComponent(Show, {
                              get when() {
                                return conn().namespace || conn().database;
                              },
                              get children() {
                                return ssr(_tmpl$78, ssrHydrationKey(), escape(conn().namespace), escape(conn().database));
                              }
                            })));
                          }
                        });
                  ...
                  })), ssrAttribute("value", escape(step.databaseUrl, true) || "", false), ssrAttribute("value", escape(step.databaseNs, true) || "", false), ssrAttribute("value", escape(step.databaseName, true) || "", false), ssrAttribute("value", escape(step.databaseUser, true) || "", false), ssrAttribute("value", escape(step.databasePass, true) || "", false));
                }
              })), escape(createComponent(Show, {
                get when() {
                  return isCustom();
                ...
                get children() {
                  return ssr(_tmpl$74, ssrHydrationKey(), ssrAttribute("value", escape(step.databaseUrl, true) || "", false), ssrAttribute("value", escape(step.databaseNs, true) || "", false), ssrAttribute("value", escape(step.databaseName, true) || "", false), ssrAttribute("value", escape(step.databaseUser, true) || "", false), ssrAttribute("value", escape(step.databasePass, true) || "", false));
                }
              })), ssrAttribute("value", escape(step.requestBodyTemplate, true) || "", false));
            })();
          }
          ...
          get children() {
            return ssr(_tmpl$31, ssrHydrationKey(), escape(createComponent(SourceStepSelect, {
              steps,
              get currentIndex() {
                return index();
              },
              ...
              get children() {
                return ssr(_tmpl$30, ssrHydrationKey(), escape(createComponent(Index, {
                  get each() {
                    return step.columns || [];
                  },
                  children: (col, ci) => ssr(_tmpl$82, ssrHydrationKey(), ssrAttribute("value", escape(col(), true), false))
                })));
              }
            })));
          }
          ...
          get children() {
            return ssr(_tmpl$43, ssrHydrationKey(), escape(createComponent(Show, {
              ...
              get fallback() {
                return ssr(_tmpl$85, ssrHydrationKey(), escape(createComponent(Show, {
                  get when() {
                    return isRunning() && !(runData()?.logs || []).some((l) => l.stepId === "auth" || l.status === "error");
                  },
                  get fallback() {
                    return ssr(_tmpl$86, ssrHydrationKey());
                  },
                  get children() {
                    return [ssr(_tmpl$83, ssrHydrationKey()), ssr(_tmpl$84, ssrHydrationKey())];
                  }
                })));
              },
              get children() {
                return [ssr(_tmpl$35, ssrHydrationKey(), `px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${log().status === "success" ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`, escape(log().status), escape(createComponent(Show, {
                  get when() {
                    return log().latencyMs;
                  },
                  get children() {
                    return ssr(_tmpl$32, ssrHydrationKey(), escape(log().latencyMs));
                  }
                })), escape(createComponent(Show, {
                  get when() {
                  ...
                  get children() {
                    return ssr(_tmpl$33, ssrHydrationKey(), `px-2.5 py-1 text-[10px] font-bold rounded-md transition-all ${resultTab() === "response" ? "bg-emerald-500/10 text-emerald-400" : "text-[#5b5b6e] hover:text-[#8b8b9e]"}`, `px-2.5 py-1 text-[10px] font-bold rounded-md transition-all ${resultTab() === "payload" ? "bg-blue-500/10 text-blue-400" : "text-[#5b5b6e] hover:text-[#8b8b9e]"}`);
                  }
                })), escape(createComponent(Show, {
                  get when() {
                    return step.type === "table" || step.type === "chart";
                  ...
                  get children() {
                    return ssr(_tmpl$34, ssrHydrationKey(), escape(stepData().length));
                  }
                }))), ssr(_tmpl$42, ssrHydrationKey(), escape(createComponent(Show, {
                  get when() {
                    return !step.type || step.type === "grpc" || step.type === "rest" || step.type === "database" || step.type === "transform";
                  },
                  get children() {
                    return [createComponent(Show, {
                      ...
                      get children() {
                        return [ssr(_tmpl$36, ssrHydrationKey()), ssr(_tmpl$37, ssrHydrationKey(), escape(JSON.stringify(log().request, null, 2)))];
                      }
                      ...
                      get children() {
                        return [ssr(_tmpl$38, ssrHydrationKey()), ssr(_tmpl$39, ssrHydrationKey(), `text-[11px] font-mono overflow-auto max-h-[220px] bg-[#0a0a0f] p-3 rounded-lg border border-[#2a2a3a]/40 custom-scrollbar whitespace-pre-wrap break-all flex-1 ${log().error ? "text-red-300 border-red-500/20 bg-red-950/5" : "text-emerald-300"}`, escape(log().error) || escape(JSON.stringify(log().response, null, 2)))];
                      }
                    })];
                  }
                })), escape(createComponent(Show, {
                  ...
                  get children() {
                    return ssr(_tmpl$40, ssrHydrationKey(), escape(createComponent(Show, {
                      get when() {
                        return stepData().length > 0;
                      },
                      get fallback() {
                        return ssr(_tmpl$87, ssrHydrationKey());
                      },
                      get children() {
                  ...
                  get children() {
                    return ssr(_tmpl$41, ssrHydrationKey(), escape(createComponent(Show, {
                      get when() {
                        return stepData().length > 0;
                      },
                      get fallback() {
                        return ssr(_tmpl$88, ssrHydrationKey());
                      },
                      get children() {
                        return createComponent(Show, {
                          get when() {
      ...
      return ssr(_tmpl$17, ssrHydrationKey(), ssrAttribute("value", escape(protoId(), true), false), escape(createComponent(Show, {
        ...
        get children() {
          return createComponent(For, {
            ...
            children: (p) => ssr(_tmpl$60, ssrHydrationKey() + ssrAttribute("value", escape(p.id, true), false), escape(p.name))
          });
        }
        ...
        get children() {
          return createComponent(For, {
            ...
            children: (conn) => ssr(_tmpl$60, ssrHydrationKey() + ssrAttribute("value", escape(conn.id, true), false), escape(conn.name))
          });
        }
      })));
    }
...
function StepAuthSettings(props) {
  ...
  return ssr(_tmpl$92, ssrHydrationKey(), `flex-1 py-1 text-xs font-bold rounded transition-all ${currentAuthType() === "none" ? "bg-blue-600 text-white shadow" : "text-[#8b8b9e] hover:text-white"}`, `flex-1 py-1 text-xs font-bold rounded transition-all ${currentAuthType() === "basic" ? "bg-blue-600 text-white shadow" : "text-[#8b8b9e] hover:text-white"}`, `flex-1 py-1 text-xs font-bold rounded transition-all ${currentAuthType() === "oauth" ? "bg-blue-600 text-white shadow" : "text-[#8b8b9e] hover:text-white"}`, escape(createComponent(Show, {
    get when() {
      return currentAuthType() === "basic";
    },
    get children() {
      return ssr(_tmpl$89, ssrHydrationKey(), ssrAttribute("value", escape(props.step?.authUsername, true) || "", false), ssrAttribute("value", escape(props.step?.authPassword, true) || "", false));
    }
  })), escape(createComponent(Show, {
    get when() {
    ...
    get children() {
      return ssr(_tmpl$91, ssrHydrationKey(), ssrAttribute("value", escape(props.step?.connectionId, true) || "", false), escape(createComponent(For, {
        get each() {
        ...
        get children() {
          return ssr(_tmpl$90, ssrHydrationKey(), escape(selectedConnection()?.tokenPath) || "access_token");
        }
      })));
    }
  })));
...
//# sourceMappingURL=_id_-DA_lpHVk.js.map
