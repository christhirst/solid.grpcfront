...
import { n as normalizeConnection, f as fetchPreRequestToken } from "./connections-DgOrSVbb.js";
import { v4 } from "uuid";
...
async function _runWorkflowBackground(workflow, runId, customContext = {}) {
  ...
  const rawWfId = workflow.id ? typeof workflow.id === "object" ? workflow.id.toString() : String(workflow.id) : "";
    ...
    for (const step of executionSteps) {
      const stepResult = await Sentry.startSpan({
        ...
        if (step.type === "grpc_stream") {
          ...
          let stepProtoContent2 = protoContent;
          if (step.connectionId) {
            try {
              ...
              if (rawConn) {
                ...
                if (!stepProtoContent2 && conn.protoId) {
                  try {
                    ...
                    if (rawProto?.content) stepProtoContent2 = rawProto.content;
                  } catch {
                  }
                }
              }
          ...
          await new Promise((resolve) => {
            executeGrpcStreamCall({
              protoContent: stepProtoContent2,
        ...
        if (step.type === "database") {
          ...
          if (typeof queryPayload === "string") {
            ...
            if (trimmed.startsWith("{") && trimmed.endsWith("}") && /^\{\s*(SELECT|CREATE|UPDATE|DELETE|INSERT|RELATE|DEFINE|REMOVE|INFO|LET|IF|FOR|RETURN|BEGIN|COMMIT|CANCEL)\b/i.test(trimmed)) {
...
async function connectWithTimeout(s, url, opts, timeoutMs = 8e3) {
  let timer;
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      ...
      reject(new Error(`SurrealDB connection to ${url} timed out after ${timeoutMs}ms.`));
    }, timeoutMs);
  });
  try {
    await Promise.race([s.connect(url, opts), timeoutPromise]);
  } finally {
    clearTimeout(timer);
  }
}
async function createConnectedClient(url, user, pass, namespace, database, connectTimeoutMs = 8e3) {
  ...
  for (const auth of authAttempts) {
    ...
    try {
      await connectWithTimeout(client, url, {
        authentication: auth
      }, connectTimeoutMs);
      return client;
    } catch (err) {
      ...
      console$1.warn(`[DB] Auth attempt failed (${Object.keys(auth).join(", ")}): ${err?.message || err}`);
    }
  }
  throw lastError;
}
...
async function getDb() {
  ...
  dbPromise = Sentry.startSpan({
    ...
    const connectTimeoutMs = Number(process.env.SURREALDB_CONNECT_TIMEOUT_MS || 8e3);
    ...
    try {
      ...
      const db2 = await createConnectedClient(url, user, pass, namespace, database, connectTimeoutMs);
      try {
...
async function getCustomDb(opts) {
  ...
  const connectTimeoutMs = opts.timeoutMs || Number(process.env.SURREALDB_CONNECT_TIMEOUT_MS || 8e3);
...
//# sourceMappingURL=db-YKr91eu6.js.map
