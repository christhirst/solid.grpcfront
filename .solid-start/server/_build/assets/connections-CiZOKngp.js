...
function normalizeConnection(raw) {
  ...
  return {
    ...
    timeoutMs: raw.timeoutMs !== void 0 ? Number(raw.timeoutMs) : void 0
  };
}
async function getCaCertContent(caId) {
  ...
  try {
    const {
      getDb
    } = await import("./db-DIgLjdSD.js").then((n) => n.d);
...
async function testSurrealDbConnection(config) {
  ...
  const timeoutMs = Number(config.timeoutMs || config.timeout || 5e3);
  if (!url) {
    return {
      success: false,
      type: "surrealdb",
  ...
  for (const auth of authAttempts) {
    try {
      const {
        client
      } = await tryConnect(auth);
      const latencyMs = Date.now() - startTime;
      let infoResult = null;
      try {
        infoResult = await client.query("INFO FOR DB;");
      } catch {
      }
      try {
        await client.close();
  ...
  return {
    ...
    url,
    latencyMs: Date.now() - startTime,
    error: `SurrealDB Connection Failed: ${lastError?.message || lastError}`
  };
}
export {
...
//# sourceMappingURL=connections-CiZOKngp.js.map
